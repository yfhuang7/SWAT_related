...
gages <- read.csv(paste("./",island,"/NWIS_00045_",island,"_pick.csv",sep = ""), stringsAsFactors = F)

## Basic set-up for the dataRetrieval
siteNumbers <- as.character(gages$site_no)

parameterCd <- "00045"  # Precipitation (in)
dataType <- "dv"  # Daily 
...
for(i in 1:length(siteNumbers)){
  pk.input <- readNWISdv(siteNumbers[i], parameterCd = parameterCd, startDate = startDate, endDate = endDate)
